...
'''
Please note that this code was sourced from the GASpy repository of 
A/Prof Zachary Ulissi's group, which can be found at 
https://github.com/ulissigroup/GASpy. 
The original version of the code was authored by Prof John Kitchin, 
and credit goes to the respective developers involved.
'''
'''
...
def __decode(jsondict):
    ...
    if isinstance(jsondict, dict):
        assert '__ndarray__' in jsondict.keys(), "__ndarray__ not provided"
        jsonstr = jsondict['__ndarray__']
        return np.array(jsonstr[-1]).reshape(jsonstr[0])
    elif isinstance(jsondict, list):
        return jsondict
    else:
        raise "Unable to decode this json infomation"
        return None
